/*
 * Download a Geometrics G856 magnetometer or something else
 * Paul Gettings
 ...
 * with spaces.  When it receives a EOT (decimal 4), it stops.
 * 
 ...
 *
 * Connector to G856 should be wired as for DTE equipment!
 *   DB25     G856    Wire color
 *   ----     ----    ----------
 *    2        T        Black
 *    3        U        White
 *    5        G        Red
...
void usage(void);
void initSerialPort(int handle, int baud, int bits);
void makeraw(struct termios *termios_p, int bits);
...
void usage(void)
{
  ...
  fprintf(stderr, "  -B BITS\tSet the serial port byte size (7 or 8 bits)\n");
  fprintf(stderr, "\n");
  fprintf(stderr, "Parity bits are ignored!\n");
  fprintf(stderr, "Transmission should end with 0x04 (ASCII EOT)\n");
}
...
int main(int argc, char *argv[])
{
  ...
  printf("G856 Download Program\n");
  printf(" Version %s\n", version);
  /* deal with command-line arguments */
  baud = B9600; strcpy(bstr, "9600");
	...
	break;
      case 'b': /* set baud rate */
        ...
        switch(br) {
          ...
          default: baud = B9600; strcpy(bstr, "9600"); break;
          }
        break;
      case 'B': /* set number of bits */
        bits = atoi(optarg);
        break;
      case 'h': /* get help */
        usage();
  ...
  printf("Using port %s at baud %s, byte size is %d bits.\n", portName, bstr, bits);
  ...
  initSerialPort(portHandle, baud, bits);
  ...
  while(run) {
    ...
    for(i=0; i<offset; i++) {
      ...
      if(record[i] == 4) { /* 4 is ASCII EOT == EOF */
        run = 0;
        }
...
/* return 1 if alphanumerics in this record */
int emptyRecord(char *rec)
{
  int i;
